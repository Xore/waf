...
.DESCRIPTION
    ...
    Administration Tools (RSAT). This approach provides maximum compatibility
    with any Windows system without additional components.
    
    Features:
    - Direct LDAP queries to domain controllers
    - Computer and user account information retrieval
    - Group membership enumeration
    - Trust relationship health monitoring
    - Domain controller connectivity testing
    - No RSAT or Active Directory module required
...
.NOTES
    ...
    - 3.0: Enhanced LDAP queries and error handling
    ...
    NinjaRMM Fields Updated:
        - adDomainJoined, adDomainName, adDomainController
        - adSiteName, adComputerOU, adComputerGroups
        - adLastLogonUser, adUserFirstName, adUserLastName, adUserGroups
        - adPasswordLastSet, adTrustRelationshipHealthy
...
process {
    try {
        ...
        if ($computerSystem.PartOfDomain -eq $false) {
            ...
            Write-Log "Workgroup: $($computerSystem.Workgroup)" -Level INFO
            ...
            Write-Log "Monitoring complete (not domain-joined)" -Level SUCCESS
            $script:ExitCode = 0
            return
        }
