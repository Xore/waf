...
.NOTES
    File Name      : AD-GetOrganizationalUnit.ps1
...
process {
    try {
        ...
        Write-Log "Computer is domain-joined" -Level SUCCESS
        ...
        try {
            ...
            if ($DistinguishedName -and $DistinguishedName.'Distinguished-Name') {
                ...
                Write-Log "Distinguished-Name property is empty" -Level ERROR
                $script:ExitCode = 1
                return
            }
        } catch {
            Write-Log "Failed to retrieve OU from Group Policy State: $_" -Level ERROR
        ...
        if ($SecureChannelTest) {
            Write-Log "Secure channel test passed - OU information is live" -Level SUCCESS
            $IsCached = "No"
        } else {
        ...
        Write-Log "OU query completed successfully" -Level SUCCESS
        $script:ExitCode = 0
