...
.NOTES
    ...
    Version: 1.1
    Release Notes: Switched to using net localgroup as it's the most reliable. Removed PowerShell 5.1 requirement.
#>
...
process {
    ...
    if (-not $Users) {
        Write-Host "[Error] No user's found! This is extremely unlikely is something blocking access to 'net localgroup administrators'?"
        # Get objects in the Administrators group, includes user objects and groups
        $Users = net.exe localgroup "Administratoren" | Where-Object { $_ -AND $_ -notmatch "command completed successfully" } | Select-Object -Skip 4
    
        if (-not $Users) {
            Write-Host "[Error] No user's found! This is extremely unlikely is something blocking access to 'net localgroup administratoren'?"
            exit 1
        }
    }
