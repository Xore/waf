    ...
    elseif ($BootEntries.BootType -contains "Unexpected Shutdown") {
        ...
        $BootEntries | ForEach-Object {
			...
			try {
				...
				$LastEvent = Get-WinEvent -FilterHashtable $EventFilter -MaxEvents 1 -ErrorAction Stop
				...
				if ($_.Exception.Message -like "*corrupted*") {
					Write-Error "CRITICAL ERROR: Event log file is corrupted. Script execution terminated."
					Write-Host "Event Log Corruption detected. Please run: wevtutil cl System" -ForegroundColor Red
					exit 1
				}
				else {
					Write-Warning "Event log error occurred: $($_.Exception.Message)"
					# Optional: Entscheiden ob Script weiterlaufen soll oder beenden
					exit 1
				}
			}
			catch {
				Write-Error "Unexpected error while accessing event log: $($_.Exception.Message)"
				exit 1
			}
        }
    }
