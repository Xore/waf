...
.NOTES
    Script Name:    Windows-CheckWin11UpgradeCompatibility.ps1
    Author:         Windows Automation Framework (Based on Microsoft HardwareReadiness.ps1)
    Version:        3.0
...
function Get-HardwareReadiness {
    ...
    # CPU Family Validation (C# Code for low-level processor checks)
    $Source = @"
...
try {
    ...
    Write-Host "Executing Windows 11 hardware readiness checks..."
    $Result = Get-HardwareReadiness | Select-Object -Unique | ConvertFrom-Json

    # Update NinjaRMM custom field
    Switch ($Result.returnCode) {
        0 { Ninja-Property-Set $CustomField "Capable" }
        1 { Ninja-Property-Set $CustomField "Not Capable" }
        -1 { Ninja-Property-Set $CustomField "Undetermined" }
        -2 { Ninja-Property-Set $CustomField "Failed To Run" }
        default { Ninja-Property-Set $CustomField "Unknown" }
    }
    ...
    if ($Result.returnReason) {
        Write-Host "Failed Requirements: $($Result.returnReason)"
    }
    
    Write-Host ""
    Write-Host "Detailed Logging:"
    Write-Host $Result.logging
    
    exit $Result.returnCode
}
catch {
    Write-Host "ERROR: Windows 11 compatibility check failed: $($_.Exception.Message)"
